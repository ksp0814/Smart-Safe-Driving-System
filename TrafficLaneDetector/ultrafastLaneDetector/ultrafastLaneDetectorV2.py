...
class UltrafastLaneDetectorV2():
	...
	def DetectFrame(self, image) :
		input_tensor = self.prepare_input(image).astype('float16')
	...
	def DrawAreaOnFrame(self, image, color=(255,191,0)) :
		...
		if(self.lanes_detected != []) :
			...
			if (list(self.lanes_detected)[1] and list(self.lanes_detected)[2]):	
				self.draw_area = True
				lane_segment_img = image.copy()
				left_lanes_points, right_lanes_points = self.adjust_lanes_points(list(self.lanes_points)[1], list(self.lanes_points)[2], self.img_height)
	...
	def process_output(pred, cfg, local_width = 1, original_image_width = 1640, original_image_height = 590):
		...
		return lanes_points.values(), lanes_detected.values()
	...
	def draw_lanes(input_img, lanes_points, lanes_detected, cfg, draw_points=True, original_image_width = 1640, original_image_height = 590):
		...
		# Draw a mask for the current lane
		if(lanes_detected != []) :
			if list(lanes_detected)[1] and list(lanes_detected)[2]:
				lane_segment_img = visualization_img.copy()
				
				lanes_points = np.array(list(lanes_points))
